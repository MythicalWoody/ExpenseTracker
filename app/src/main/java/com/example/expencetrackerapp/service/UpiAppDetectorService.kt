...
class UpiAppDetectorService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // IGNORE Samsung Pay background overlay (Quick Access)
        // It reports as "android.widget.FrameLayout" instead of an Activity
        if ((packageName == "com.samsung.android.spay" ||
                        packageName == "com.samsung.android.samsungpay") &&
                        className == "android.widget.FrameLayout"
        ) {
            Log.d(TAG, "⏭️ Ignoring Samsung Pay overlay: $className")
            return
        }
    ...
    /**
     * Capture screenshot using Accessibility API (Android R+). This is INSTANT - no permission
     * dialog needed!
     */
                            ...
                            try {
                                ...
                                if (bitmap != null) {
                                    ...
                                    val file =
                                            File(
                                                    cacheDir,
                                                    "upi_screenshot_${System.currentTimeMillis()}.png"
                                            )
